...
### 方法一：直接下載單一檔案（最簡單）
*Method 1: Download a single file (easiest)*
...
### 方法二：下載整包 ZIP
*Method 2: Download the whole repo as ZIP*
...
```
PS2 receiver:  CLK→GPIO4  CMD→GPIO5  SEL→GPIO6  DAT→GPIO7
               VCC→3.3V   GND→GND        ← 3.3V only! 5V will kill it

Left wheel (L298N):  IN1→GPIO0  IN2→GPIO1
Right wheel (L298N): IN3→GPIO20 IN4→GPIO21
               GND→GND (common ground!)  Power→battery pack
               Keep the ENA/ENB jumpers on

Pickup roller (M3): Signal→GPIO3  VCC→battery  GND→GND (common ground!)
```

**共地是什麼？**
訊號是「相對電壓」，馬達模組和開發板的 GND 一定要接在一起，不然馬達不聽話。

*What is common ground?*
*Signals are relative voltages — the GND of every module must be connected to the board's GND, or the motors will not respond.*

---

## 五、操作方式（主課程版）
*Controls (main sketch)*

| 操作<br>*Control* | 動作<br>*Action* |
|------|------|
| 左搖桿<br>*Left stick* | 控制左輪（推 = 前轉、拉 = 反轉）<br>*Left wheel (up = forward, down = reverse)* |
| 右搖桿<br>*Right stick* | 控制右輪<br>*Right wheel* |
| 兩支一起推<br>*Both sticks up* | 車子前進<br>*Car moves forward* |
| 按住 L1<br>*Hold L1* | 撿球滾刷轉動<br>*Pickup roller spins* |
| 放開 L1<br>*Release L1* | 滾刷停止<br>*Roller stops* |
...
| | 雙搖桿版（主課程）<br>*Dual-stick (main)* | 右搖桿版（挑戰）<br>*Right-stick (challenge)* |
|--|-------------------|-----------------|
| 開車方式<br>*Driving* | 左搖桿=左輪、右搖桿=右輪<br>*Left stick = left wheel, right stick = right wheel* | 右搖桿一支開車（Y 前進、X 轉向）<br>*Single right stick (Y = throttle, X = steering)* |
| 滾刷<br>*Roller* | L1 按鈕開/關（GPIO3 一條線）<br>*L1 on/off (single wire on GPIO3)* | 左搖桿調速正反轉（GPIO3 + GPIO10 兩條線）<br>*Left stick, bidirectional speed (two wires: GPIO3 + GPIO10)* |
...
1. **入門**：把 `* 2` 改成 `* 3` → 車子變快還是變慢？
   *Beginner: change `* 2` to `* 3` → is the car faster or slower?*
2. **入門**：把死區 `20` 改成 `50` → 搖桿要推更多車子才動
   *Beginner: change the deadzone `20` to `50` → the stick must be pushed further before the car moves.*
3. **進階**：燒錄 `PS2X_RightStick_ESP32.ino`（單搖桿開車版，M3 要多接一條線到 GPIO10）
   *Advanced: flash `PS2X_RightStick_ESP32.ino` (single-stick drive; M3 needs one extra wire to GPIO10).*
4. **魔王**：把 M3 的 L1 改成別的按鈕（提示：改 `PSB_L1`，可換成 `PSB_CIRCLE`）
   *Expert: change M3's L1 to another button (hint: change `PSB_L1`, e.g. to `PSB_CIRCLE`).*
...
- 函式庫作者：Bill Porter
  *Library author: [Bill Porter](http://www.billporter.info/)*
- 最早的 PS2 控制器程式碼：Shutter（Arduino 論壇）
  *Original PS2 controller code: Shutter (Arduino Forum)*
- 本專案新增：ESP32-C3 腳位設定、L298N 馬達控制、撿球滾刷控制
  *Added in this project: ESP32-C3 pin mapping, L298N motor control, pickup roller control*
