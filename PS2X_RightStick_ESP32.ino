...
// Modified: ESP32-C3 pin mapping, single-stick (arcade) drive, L298N wheel motor control,
//           bidirectional pickup roller (TT motor) control.

#include <PS2X_lib.h>  // for v1.6

/******************************************************************
 * ESP32-C3 pin assignment:
 *   Control: right stick = drive (Y = forward/backward, X = left/right turn)
 *            left stick Y = TT motor (up = forward, down = reverse)
 ...
 *   TT motor (M3) -> IN1(GPIO3) / IN2(GPIO10) on its own motor module
...
#define MOTOR3_A       3   // TT motor IN1
#define MOTOR3_B       10  // TT motor IN2

// left motor = IN1/IN2, right motor = IN3/IN4 (L298N style driver)
// M3 = its own motor module, IN1/IN2 as well
...
void loop() {
  ...
  if (type == 1 || type == 3) {  // DualShock (wired) or Wireless Sony DualShock
    ...
    // --- wheel motors: right stick only ---
    // Y = forward/backward, X = left/right turn
    // stick: 0 = fully down/left, 128 = center, 255 = fully up/right
    int spdY = constrain((ps2x.Analog(PSS_RY) - 128) * 2, -255, 255);
    int spdX = constrain((ps2x.Analog(PSS_RX) - 128) * 2, -255, 255);
    int spdL = constrain(spdY + spdX, -255, 255);
    int spdR = constrain(spdY - spdX, -255, 255);
    ...
    // --- third motor (TT motor): left stick Y, up = forward, down = reverse ---
    int spdM3 = constrain((ps2x.Analog(PSS_LY) - 128) * 2, -255, 255);
    if (abs(spdM3) < 20) spdM3 = 0;  // deadzone
    setMotor(MOTOR3_A, MOTOR3_B, spdM3);
